...
centroids <- readRDS(here::here("prepped","grids_hex_col_hex25.rds")) %>%
  dplyr::filter(countrypoly == 1) %>%
  mutate(longitude = X,
         latitude = Y) 

# table(centroids$countrypoly)
events <- readRDS(here::here("prepped","events_col_hex25.rds")) %>%
...
saveRDS(df_s_t_trunc, here::here("prepped","events_decay_col_hex25_logistic_trunc.rds"))
