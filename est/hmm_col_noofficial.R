...
# HMM analysis
# Colombia (Robustness check, excluding all offical targets)
#
# Therese Anders
############################
...
events_raw <- readRDS(here::here("prepped","events_col_noofficial_hex25.rds")) %>%
  st_as_sf() %>%
  filter(colwest == 1)

gid_sub <- unique(grid_og)

events <- readRDS(here::here("prepped","events_decay_col_noofficial_hex25_logistic_trunc.rds"))
...
saveRDS(hmm_grid, here::here("prepped","hmm_col_noofficial.rds"))
